...
N_FLOP_BUCKETS = 10
N_TURN_BUCKETS = 50
EQUITY_SAVE_NAME = 'poker_flop_equities.pkl'
FLOP_SAVE_NAME = 'poker_flop_abstraction.pkl'
TURN_SAVE_NAME = 'poker_turn_abstraction.pkl'
...
def headsup_equity(hole, opponent_hole, flop):
    ...
    for turn in get_deck():
        if turn not in (hole, opponent_hole, flop):
            player_hand = Hand(hole, flop, turn)
            opponent_hand = Hand(opponent_hole, flop, turn)
...
def turn_equity(hole, flop, turn):
    ...
    for opponent_card in get_deck():
        if opponent_card not in (hole, flop, turn):
            player_hand = Hand(hole, flop, turn)
            opponent_hand = Hand(opponent_card, flop, turn)
            if player_hand > opponent_hand:
                equity += 1
            elif player_hand == opponent_hand:
                equity += 0.5
